        ...
        <section className="w-full bg-white py-8 md:py-16 px-4">
            <div className="max-w-7xl mx-auto">
                <div className="text-center mb-6 md:mb-10">
                    <h2 className="text-xl md:text-4xl font-serif text-[#8B7355] italic mb-2 uppercase tracking-wide">
                        GET STARTED WITH OUR 5 STEP EASY PROCESS
                    </h2>
                    <h3 className="text-sm md:text-2xl font-serif text-[#8B7355] italic uppercase">
                        {currentStepData.title}
                    </h3>
                </div>

                <div className="relative flex flex-col lg:flex-row items-center justify-center gap-8 max-w-5xl mx-auto">
                    {/* Controls */}
                    <div className="flex items-center justify-between w-full md:w-auto md:absolute md:inset-x-[-100px] md:top-1/2 md:-translate-y-1/2 z-20 mb-4 md:mb-0">
                        <button
                            onClick={handlePrev}
                            className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-white shadow-xl flex items-center justify-center border border-gray-100 hover:border-[#8B7355] transition-all"
                        >
                            <svg className="w-6 h-6 text-[#8B7355]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                        </button>

                        <div className="flex gap-1 lg:hidden">
                            {steps.map((_, i) => (
                                <div key={i} className={`h-1.5 rounded-full transition-all ${i === currentStep ? 'w-6 bg-[#8B7355]' : 'w-2 bg-gray-200'}`}></div>
                            ))}
                        </div>
                        ...
                        <button
                            ...
                            className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-white shadow-xl flex items-center justify-center border border-gray-100 hover:border-[#8B7355] transition-all"
                        >
                            <svg className="w-6 h-6 text-[#8B7355]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                        </button>
                    </div>
                    ...
                    <div className="w-full lg:w-1/2">
                        <div className="relative aspect-video rounded-3xl overflow-hidden shadow-2xl border-4 border-white">
                            ...
                            <div className="absolute top-4 left-4 bg-[#8B7355] text-white w-10 h-10 rounded-full flex items-center justify-center font-bold shadow-lg">
                                {currentStepData.id}
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Content */}
                    <div className="w-full lg:w-1/2">
                        <div className="bg-gray-50/50 p-6 md:p-10 rounded-3xl border border-gray-100 shadow-sm relative">
                            <div className="absolute -top-4 -left-4 w-12 h-12 bg-[#8B7355]/10 rounded-full"></div>
