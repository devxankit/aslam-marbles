...
const LimitedEditionPage = ({ onShowCart, onShowLikes }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch(`${API_URL}/special-products/limited-edition`)
      ...
      <div className="w-full py-12 md:py-16 px-4 md:px-6 lg:px-8 bg-[#FDFBF7]">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="text-3xl md:text-5xl font-serif text-[#8B7355] italic mb-4 font-bold tracking-tight">
            <TranslatedText>Limited Edition</TranslatedText>
          </h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
              {products.length === 0 ? (
                <div className="col-span-full text-center text-gray-500 py-10">
                  <TranslatedText>No limited edition products available.</TranslatedText>
                </div>
              ) : products.map((product) => (
                <div
                  key={product._id || product.id}
                  onClick={() => navigate(`/limited-edition/limited-series/${product._id || product.id}`)}
                ...
                >
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    <div className="mb-2">
                      <span className="text-[10px] uppercase tracking-[0.2em] text-[#8B7355] font-bold"><TranslatedText>Rare Masterpiece</TranslatedText></span>
                    </div>
